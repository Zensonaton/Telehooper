...
SETTINGS_TREE = {
	...
	"Services": {
		...
		"VK": {
			...
			"GroupInfoCopy": {
				"Name": "Копирование параметров диалога",
				"Documentation": (
					"Telehooper при подключении диалога сервиса к группе Telegram автоматически копирует следующие свойства диалога, что бы быть группа Telegram была похожей на диалог из сервиса:\n"
					" • Имя диалога,\n"
					" • Фотография диалога,\n"
					" • Описание диалога.\n"
					"\n"
					"Выключив данную настройку, бот не будет менять данные параметры группы в Telegram после подключения к диалогу." # TODO: Ссылка на настройку для синхронизации параметров уже "живущих" диалогов.
				),
				"Default": True
			}
		}
