...
async def VKDialogueSelector(query: CallbackQuery) -> bool:
	...
	await query.message.answer(f"<b>Группа-диалог</b> 🫂\n\nПрекрасно! Эта группа теперь является диалогом с <b>«{dialogue.fullName}»</b>! Все сообщения, отправляемые здесь, будут отправляться в диалог <b>{dialogue.fullName}</b>. Настроить этот диалог можно снова воспользовавшись командой /this.\nПриятного общения! 😊")
