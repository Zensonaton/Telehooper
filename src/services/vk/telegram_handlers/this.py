...
async def this_vk_convert_inline_handler(query: CallbackQuery, msg: Message, user: User, queryStr: str, bot: Bot) -> None:
	...
	if (dialog.multiuser_count or 0) >= 3:
		docs_info.append("")
		docs_info.append(f"В беседе ВКонтакте «{dialog.name}», которую Вы соединили с этим диалогом, находится {dialog.multiuser_count} участников. Telehooper совершенно спокойно справится с таким количеством пользователей, однако велика вероятность что Вам будет неудобно воспринимать сообщения поскольку все они будут отправляться от имени одного бота, Telehooper. Если это и вправду будет неудобством в будущем, то хорошей идеей будет прочитать про «мини-ботов»: <a href=\"{MINIBOTS_INFO_URL}\">🔗 ссылка</a>.")
