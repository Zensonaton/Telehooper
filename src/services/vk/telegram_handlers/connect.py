...
async def connect_vk_token_handler(msg: types.Message) -> None:
	...
	try:
		...
		_text = (
			...
			"\n"
			"ℹ️ Теперь Вы можете «соединить» диалог или группу из ВКонтакте в Telegram; Для этого Вам нужно создать группу в Telegram и добавить туда этого бота. Для более подробной информации обратитесь к команде <code>/help 5</code>.\n"
		)

		if not allow_tokens_storing:
			_text += (
				"\n"
				"⚠️ Предупреждение: Вы запретили боту сохранять токены авторизации в базу данных при помощи настройки <i>⚙️ Хранение токенов в БД</i> (<code>/s Security.StoreTokens</code>).\n"
				...
				"Если Вам такое поведение не нравится, то Вам нужно разрешить боту сохранять токены в БД, поставив значение «включено» у настройки <i>⚙️ Хранение токенов в БД</i> (<code>/s Security.StoreTokens</code>)."
			)
